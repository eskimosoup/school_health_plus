class ApplicationController < ActionController::Base
  ...
  def accessibility
    ...
    @branches  = []
    @branches += Willow::Tree.find_by_name('Top').branches.roots.order(:access_key)
    @branches += Willow::Tree.find_by_name('Main').branches.roots.order(:access_key)
  end
  
  def site_map
		@branch   = Willow::Branch.find_by_name("Site Map")
    @branches = Willow::Tree.find_by_name('Main').branches.order(:positions_depth_cache)
  end
